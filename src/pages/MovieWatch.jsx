...
function MovieWatch() {
  ...
  const isHlsStream = typeof streamingUrl === 'string' && streamingUrl.includes('.m3u8');
  ...
  const resolveStreamingUrl = async (movieData, hasUserAccess) => {
    try {
      const cached = localStorage.getItem('streamingData');
      if (cached) {
        const parsed = JSON.parse(cached);
        if (parsed?.movieId === movieData.id) {
          return parsed.hlsUrl || parsed.streamingUrl || null;
        }
      }
    } catch (e) {
      // Ignore bad cache and continue with API flow.
    }
    ...
    try {
      const signed = await moviesService.getSignedStreamUrl(movieData.id);
      const signedUrl = signed?.data?.data?.streamUrl;
      if (signedUrl) {
        return normalizeStreamUrl(signedUrl);
      }
    } catch (e) {
      // Stream might not be ready yet. Fall back to direct URLs.
    }

    return movieData.videoUrl || movieData.streamingUrl || movieData.hlsUrl || null;
  };
  ...
  const fetchMovie = async () => {
    try {
      ...
      const userHasAccess = Boolean(
        movieData.userAccess?.hasAccess ||
        user?.role === 'admin' ||
        movieData.viewPrice === 0
      );
      setHasAccess(userHasAccess);
      const resolvedUrl = await resolveStreamingUrl(movieData, userHasAccess);
      setStreamingUrl(resolvedUrl);
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          {playerState.playerError ? (
            ...
            <>
              ...
              >
                <source src={streamingUrl} type={isHlsStream ? "application/x-mpegURL" : "video/mp4"} />
